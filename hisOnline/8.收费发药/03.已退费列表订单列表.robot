...
...    AND    退费     orderNo=${orderNo}    orderExamList={"orderExamListId":${orderExamListId},"orderExams":[{"itemId":${examItemId},"count":${examCount}}]}    orderPrescriptions={"orderPrescriptionId": ${orderPrescriptionId},"presciptionType": "1","orderDrugs": [{"itemId": ${prescriptionItemId},"count": ${prescriptionCount}}]}    balanceId=${null}    refundAmt=${actualAmt}    recordVersion=${recordVersion}    debtAmt=0    payMode=3
...
3.1 已退费列表订单列表
    ...
    ${getRes}    已退费列表订单列表    startDate=2013-05-03    endDate=${today}
    ...    patientName=    doctorId=    currentPage=    pageSize=50
    # ${aj}    Evaluate    [aj['patientName'] for aj in $getRes['body']['orders']]
    Should Contain    ${getRes['body']['orders'][0]['patientName']}    接口自动
    # Should Contain    ${aj}    z311
