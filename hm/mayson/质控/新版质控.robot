...
单病种质控:急性心肌梗死,出院记录,推出治疗方案:Killip分级
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    # ${timestamp}    Get Time    epoch
    ${Assessment}    Set Variable    急性心肌梗死
...
单病种质控:社区获得性肺炎-成人,出院记录,推出检查:痰培养
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    # ${timestamp}    Get Time    epoch
...
单病种质控:慢性阻塞性肺疾病急性加重,推出评估表:慢性阻塞性肺炎的综合评估
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    # ${timestamp}    Get Time    epoch
