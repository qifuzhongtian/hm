...
先天性胆管扩张症-手动输入诊断：先天性胆管扩张症，推荐检查:尿常规、粪便常规、血常规、C-反应蛋白、血型、血生化、凝血常规、乙肝五项、血气分析、胸部X线片（正位）、心电图、超声心动图、腹部B超、上腹部CT、磁共振胰胆管造影（MRCP)、CT三维重建、经内镜逆行胰胆道造影(ERCP)
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    经内镜逆行胰胆道造影(ERCP)    上腹部CT    CT三维重建    磁共振胰胆管造影（MRCP)    腹部B超    超声心动图    尿常规    乙肝五项    胸部X线片（正位）    血型    粪便常规    C-反应蛋白    血气分析    血常规    凝血常规    心电图    血生化
...
先天性胆管扩张症-手动输入诊断：先天性胆管扩张症，推荐治疗:扩张胆总管切除术、胆道重建术、内膜剥除术、预防性抗菌药物治疗、术后抗菌药物治疗
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    内膜剥除术    预防性抗菌药物治疗    术后抗菌药物治疗    扩张胆总管切除术    胆道重建术
    ${Assessment}    Set Variable    先天性胆管扩张症
    ${Subjective}    Set Variable    男，10岁，腹痛，哭闹不止，恶心、呕吐，皮肤颜色变黄,检查结果示：间歇性黄疸加重，右上腹囊性包块，尿胆红素阳性
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
...
先天性胆管扩张症-增加病历内容：胆总管局部粘连重，推荐治疗增加:囊肿外引流术+囊肿切除术+胆道重建术
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    内膜剥除术    囊肿外引流术+囊肿切除术+胆道重建术    预防性抗菌药物治疗    术后抗菌药物治疗    扩张胆总管切除术    胆道重建术
    ${Assessment}    Set Variable    先天性胆管扩张症
    ${Subjective}    Set Variable    男，10岁，腹痛，哭闹不止，恶心、呕吐，皮肤颜色变黄,检查结果示：间歇性黄疸加重，右上腹囊性包块，尿胆红素阳性,胆总管局部粘连重
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    ${getRes}    智能推荐    userGuid=${timestamp}${random}    serialNumber=${timestamp}${random}
    ...    patientInfo={"gender":"1","age":"10","ageType":"岁","maritalStatus":"1","pregnancyStatus":"0"}
