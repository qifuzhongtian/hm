...
丙型肝炎-主诉:男，45岁，丙肝病史15年，复查HCV抗体阳性，推荐疑似诊断:丙型肝炎
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${Assessment}    Set Variable
    ${Subjective}    Set Variable    男，45岁，丙肝病史15年，复查HCV抗体阳性
    ...
    Should Contain    ${aj[:5]}    丙型肝炎


丙型肝炎-点击疑似诊断或手动输入：丙型肝炎，推荐检查:血常规、乙肝五项、HBV血清学检测、丙型肝炎病毒基因型、肝纤维化检测、瞬时弹性成像技术(TE)、核磁共振弹性成像、肝组织取活检、肝功能、腹部超声、艾滋病毒抗体检测、甲型肝炎病毒(HAV)血清学检查、丙型肝炎病毒(HCV)抗体筛查、RNA定量检测
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    丙型肝炎病毒(HCV)抗体筛查    RNA定量检测    肝纤维化检测    乙肝五项    艾滋病毒抗体检测    血常规    肝功能    HBV血清学检测    腹部超声    核磁共振弹性成像    瞬时弹性成像技术(TE)    甲型肝炎病毒(HAV)血清学检查    丙型肝炎病毒基因型    肝组织取活检
...
丙型肝炎-点击疑似诊断或手动输入：丙型肝炎，推荐治疗:转诊
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    # ${assert}    Create List    转诊
    ${Assessment}    Set Variable    丙型肝炎
    ${Subjective}    Set Variable    男，45岁，丙肝病史15年，复查HCV抗体阳性
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
