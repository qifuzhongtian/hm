...
儿童哮喘-点击疑似诊断或手动输入：哮喘,推荐检查:胸部X线、肺通气功能测试、支气管激发试 验、哮喘控制测试
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    肺通气功能测试    支气管激发试验    胸部X线    哮喘控制测试
...
儿童哮喘-点击疑似诊断或手动输入：哮喘,推荐评估表：5~18岁儿童哮喘分类(梅奥版)
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    胸部X线    肺通气功能测试    支气管激发试验    哮喘控制测试
    ${Assessment}    Set Variable    哮喘
    ${Subjective}    Set Variable    女，10岁，反复咳喘1年，加重3天 ，双肺闻及哮鸣音
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    ...
    #####疑似诊断
    # ${aj}    Evaluate    [aj['diseaseName'] for aj in $getRes['body']['diseaseHospitalList']]
    Should Contain    ${aj[:5]}    5~18岁儿童哮喘分类(梅奥版)
    # Lists Should Be Equal    ${aj}    ${assert}



儿童哮喘-点击疑似诊断或手动输入：哮喘,推荐治疗：预防治疗
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    # ${assert}    Create List    胸部X线    肺通气功能测试    支气管激发试验    哮喘控制测试
    ${Assessment}    Set Variable    哮喘
    ...
    #####疑似诊断
    # ${aj}    Evaluate    [aj['diseaseName'] for aj in $getRes['body']['diseaseHospitalList']]
    Should Contain    ${aj[:5]}    预防治疗
    # Lists Should Be Equal    ${aj}    ${assert}


儿童哮喘-病历内容增加：运动性哮喘,推荐治疗增加：运动性哮喘的药物治疗
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    运动性哮喘的药物治疗    预防治疗
    ${Assessment}    Set Variable    哮喘
    ${Subjective}    Set Variable    女，10岁，反复咳喘1年，加重3天 ，双肺闻及哮鸣音,运动性哮喘
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    ${getRes}    智能推荐    userGuid=${timestamp}${random}    serialNumber=${timestamp}${random}
    ...    patientInfo={"gender":"0","age":"12","ageType":"岁","maritalStatus":"1","pregnancyStatus":"0"}
...
儿童哮喘-病历内容进一步增加：应用沙丁胺醇无效,推荐治疗增加：转诊
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    运动性哮喘的药物治疗    转诊    预防治疗
    ${Assessment}    Set Variable    哮喘
    ${Subjective}    Set Variable    女，10岁，反复咳喘1年，加重3天 ，双肺闻及哮鸣音,运动性哮喘,应用沙丁胺醇无效
    [Setup]    Run Keywords    获取时间戳
    ...    AND    获取随机数
    ${getRes}    智能推荐    userGuid=${timestamp}${random}    serialNumber=${timestamp}${random}
    ...    patientInfo={"gender":"0","age":"12","ageType":"岁","maritalStatus":"1","pregnancyStatus":"0"}
