...
巴雷特食管-病史：男，59岁，持续反胃数月，反酸3个月，呃逆，上腹部不适，体重减轻,推荐疑似诊断:巴雷特食管
    [Documentation]    断言:""
    ...
    Should Contain    ${aj[:5]}    巴雷特食管
    # Lists Should Be Equal    ${aj}    ${assert}


巴雷特食管-点击疑似诊断手动输入：巴雷特食管,推荐检查:食管胃十二指肠镜检查、胃组织活检
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    胃组织活检    食管胃十二指肠镜检查
...
巴雷特食管-点击疑似诊断手动输入：巴雷特食管,推荐治疗:抗反流治疗、抗反流手术
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
...
巴雷特食管-病史：明确增生程度：轻度异型增生,推荐治疗增加:质子泵抑制剂(PPI)治疗、专科确诊、内镜下射频消融术,删除:抗反流治疗
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
...
巴雷特食管-病史：进一步添加并发症：食管腺癌,推荐检查增加:正电子发射计算机断层显像(PET-CT)、胸部CT
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
    ${assert}    Create List    胸部CT    正电子发射计算机断层显像(PET-CT)    胃组织活检    食管胃十二指肠镜检查
    ${Assessment}    Set Variable    巴雷特食管
...
巴雷特食管-病史：进一步添加并发症：食管腺癌,推荐治疗增加:转诊、内镜下治疗
    [Documentation]    断言:""
    # ${timestamp}    Get Time    epoch
