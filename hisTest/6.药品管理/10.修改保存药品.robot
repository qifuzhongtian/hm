...
*** Test Cases ***
10.2 新增已存在的药品
    [Documentation]    断言:"改名成功error=0"
    ${getRes}    修改保存药品    id=${drugId}    drugName=${catenate}改名    drugType=2    isOtc=0    status=1
    ...    customType=    drugBatchNo=${secs}    expirationOptions=    approvalNumber=国药准字Z22024464    barCode=barCode    singleDosage=1
    ...    singleDosageUnit=粒    frequency=    prescribeAmount=    prescribeAmountUnit=盒    instruction=口服    doctorAdvice=先煎
    ...    specification=片*36/盒    specMinimumDosage=    specMinimumUnit=    specUnit=    specUnitaryRatio=    specPackageUnit=盒
    ...    manufacturer=吉林省健今药业有限责任公司    purchasePrice=    prescriptionPrice=99.98    inventoryCount=    inventoryThreshold=11    validityDate=4102329600000
