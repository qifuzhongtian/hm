...
妇产科室演示病例15例-1.1
    ...
    ${getRes}    智能诊断2.3    symptom=34＋3周孕，单胎，不明原因阴道流液1小时，量中，湿透外裤，不伴腹痛.查体：四步触诊检查：胎位为头位，摇动胎头时见有清亮液体自阴道口流出。外裤大片湿透，外阴湿润，无明显宫缩。
    ...    previousHistory=    personalHistory=G2P1
    ...    allergyHistory=    familyHistory=
    ...    weight=    gender=0    bodyTempr=    lowBldPress=    highBldPress=
    ...    examInfo=    heartRate=    age=26    ageType=岁    confirmDiagnosis=
    ...    confirmDiagnosisMap=    presentHistory=
    ${aj}    Evaluate    [aj['diseaseName'] for aj in $getRes['body']['diseaseGroups'][0]['diseases']]
...
妇产科室演示病例15例-2
    ...
    ${getRes}    智能诊断2.3    symptom=月经紊乱2年，伴阵发性潮热、多汗、心悸、烦躁。近2年月经周期长短不等，经量偏多，淋漓不尽，曾用中药调经效果不佳，末次月经淋漓不尽至今未净。15年前安放宫内节育器（I UD）。
    ...    previousHistory=    personalHistory=G2P1，末次月经40日前。
    ...    allergyHistory=    familyHistory=
    ...    weight=    gender=0    bodyTempr=    lowBldPress=    highBldPress=
